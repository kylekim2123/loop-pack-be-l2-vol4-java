...
@Component
public class ProductMetricsLikeCountReconcileTasklet implements Tasklet {

    private static final String RECONCILE_SQL = """
        UPDATE product_metrics m
        LEFT JOIN (
            SELECT product_id, COUNT(*) AS like_count
            FROM likes
            WHERE deleted_at IS NULL
            GROUP BY product_id
        ) src ON src.product_id = m.product_id
        SET m.like_count = COALESCE(src.like_count, 0), m.updated_at = NOW(6)
        WHERE m.like_count <> COALESCE(src.like_count, 0)
        """;
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) {
        ...
        log.info("product_metrics.like_count 원천(likes COUNT) 수렴 보정 완료 - 보정 행 수: {}", reconciledCount);
