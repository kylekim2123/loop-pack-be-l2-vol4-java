...
### ⑧ 예상 대기 시간 계산 → **순번 ÷ 발급 속도, 서버는 초 단위 숫자로**

```
예상 대기 시간 = 내 순번 ÷ 초당 발급 인원 (올림)
   예: 512번째 ÷ 초당 140명 ≈ 4초
```
정확한 값이 아니라 상황에 따라 달라지는 추정이에요. 처음엔 서버가 "약 3분" 같은 어림 문자열을 만들려 했지만, Stage 5에서 **서버는 초 단위 숫자(`estimatedWaitSeconds`)만 계산해 주고, "약 N초/분" 어림 표기는 클라이언트 몫**으로 바꿨습니다 — 서버는 계산, 표기는 화면의 책임이라 API 계약이 단순해져요.
...
- [x] **기본 계산** — 실제 DB 커넥션 수(`jpa.yml`의 `maximum-pool-size` = **40**)와 주문 1건 처리 시간으로 초당 처리량을 구하고, 안전하게 70%만 쓴 뒤, 0.1초당 인원으로 나눔 → **14명/0.1초 (140건/초)**
  ```
  초당 처리량 = 커넥션 수 ÷ 주문 1건 시간
  안전 처리량 = 초당 처리량 × 0.7
  배치 크기   = 안전 처리량 ÷ 10   (0.1초마다 발급하니까)
  ```
- [x] **쌓임 검증 (Little's Law)** — 입장권이 5분 사는 동안 "주문 가능한 사람"이 얼마나 쌓이는지 계산하고, 그게 몰려도 DB가 버티는지 확인. 넘치면 발급 속도를 낮추거나 만료 시간을 줄임 → **발급 140/s × TTL 300s = 42,000명 무장(순간 폭주 위험 확인)**
  ```
  쌓인 사람 = 발급 속도 × 만료 시간(5분)
  → 이 사람들이 몰려도 DB가 버티나? 아니면 조정
  ```
- [x] 계산 결과(배치 크기·만료 시간·주기)와 과정을 `docs/volume-8/reports/batch-sizing.md`에 기록 (Stage 7에서 실측으로 다시 맞출 예정)
...
- [x] **0.1초마다 도는 스케줄러** — `@Scheduled(fixedDelayString = "${queue.issue-interval}")`. Stage 2에서 정한 인원만큼 발급 (`fixedDelay` = 이전 작업이 끝난 뒤 0.1초 쉬고 다음 실행 → 작업이 몰리지 않음). `EntryTokenScheduler`, 배치 크기는 `queue.batch-size`(@ConfigurationProperties). 발급 주기는 Stage 5에서 `queue.issue-interval`(100ms) 설정으로 옮겨 예상 대기 계산과 단일 원천 공유
- [x] **꺼내는 순서: 보기(Lua 밖) → 발급+삭제(Lua)** — ① 앞 N명 조회(`ZRANGE`)는 Lua 밖에서(읽기라 죽어도 무해) ② 각자 UUID를 만들어 **발급(`SET`)과 줄 삭제(`ZREM`)를 Lua 한 덩어리로 원자 실행**(유저 1명당 Lua 1회). 발급됐는데 삭제 안 되는 어중간한 상태가 없어 증발도, 중복 발급도 안 생김
- [x] **입장권 = 랜덤 문자열 + 5분 만료** — `entry-token:{userId}`에 UUID 저장, `queue.token-ttl`(300초) 뒤 자동 삭제
- [x] 줄에서 지우는 건 발급과 같은 Lua 안에서 함께 (나중으로 미루지 않기)
- [x] 발급은 주(master) 저장소에서 (`masterRedisTemplate`)
...
- [x] **검문소(`EntryTokenInterceptor`)** — 주문 API(`POST /orders`)에 적용(기존 `AdminAuthInterceptor` 본뜸). POST만 게이트, loginId→userId 식별(미식별 게스트는 스킵→리졸버 401). 헤더에 입장권 없으면 `FORBIDDEN`("입장권이 필요합니다.")
- [x] **Lua로 "확인 + 삭제"를 한 덩어리로** — 저장된 입장권이 헤더 값과 같으면 삭제하고 통과, 아니면 차단(`FORBIDDEN` "유효하지 않은 입장권입니다."). 더블클릭 시 딱 한 번만 통과. Lua는 남은 TTL을 회수해 반환
  ```lua
  ...
  return 0                                     -- 없거나 다르면 차단
  ```
- [x] **주문 실패 시 되살리기** — `afterCompletion`에서 응답 status ≥ 400이면 검문소가 입장권을 다시 저장(재시도 허용). 인증 실패 포함 2xx 아닌 모든 경우를 덮되, **새 TTL이 아니라 소비 때 회수한 남은 TTL로 복구**(무한 연장 방지). 성공(2xx)이면 소진 유지
- [x] 확인·삭제는 주(master) 저장소에서 (복사본 시간차로 잘못 막히는 것 방지)
- [x] 주문 처리 코드(`OrderFacade`)는 안 건드림 — 검문은 인터셉터에만
...
- [x] **예상 대기 시간** — `내 순번 ÷ 초당 발급 인원`(올림, 최소 1초). 응답은 `estimatedWaitSeconds` 숫자(초)로, "약 N초/분" 어림 표기는 클라이언트 몫 (결정 ⑧ 갱신). 초당 발급 인원은 `batch-size ÷ issue-interval`로 계산 — 발급 주기를 스케줄러 상수에서 `queue.issue-interval`(@ConfigurationProperties)로 옮겨 스케줄러와 같은 값을 봄
- [x] **순번 조회 응답 분기** — 두 경우로 나뉨:
  ```
  입장권이 발급돼 있으면  → { position: 0, entryToken: "a3f9-..." }
  아직 대기 중이면        → { position: 512, totalWaiting: 1200, estimatedWaitSeconds: 4 }
  ```
  입장권 받은 사람은 이미 줄에서 빠졌으니(Stage 3), "입장권 있나?"를 먼저 확인. 둘 다 없으면 기존대로 404
- [x] 응답은 필드가 정해진 record 형태로 — `PositionResponse` 단일 record + 해당 없는 필드는 null (Jackson NON_NULL이라 응답 JSON에서 빠짐)
...
- [x] **동시 진입 순서** — 동시에 N명 진입 → 전원 수용 + 완료 후 순번이 1..N 정확히 한 번씩(유일), 같은 유저는 한 번만. 응답 속 순번은 진입 도중의 스냅샷이라 중복될 수 있어(결정 ①의 같은 밀리초 감수와 같은 맥락) 완료 후 조회로 단언
- [x] **입장권 만료** — 짧은 TTL(1초) 입장권을 직접 시드하고 `Awaitility`로 소멸을 기다린 뒤 주문 → 403 차단 확인 (고정 sleep 없음)
- [x] **더블클릭** — 같은 입장권 동시 2회 → 201·403 하나씩, 주문 1건, 입장권 소진
- [x] **처리량 초과** — 배치 크기 2배+7명 시드 → 주기마다 앞에서 배치만큼만 발급되고 초과분은 대기열 잔존, 3주기에 전원 소진 (배치 크기는 `QueueProperties`에서 읽어 Stage 7 튜닝에도 유효)
- [x] E2E 테스트 — 진입 → 발급 → 순번 조회(입장권 수령) → 주문 → 입장권 소진 → 재조회 404까지 관통. 에러는 상태코드·결과·에러코드까지 확인
- [x] 다섯 시나리오를 `QueueVerificationE2ETest` 한 클래스에 모음 (Stage 6 검증 전용)

**이 단계 완료 기준:** 세 가지 요구 테스트가 통과한다. → 모듈 전체 739개 통과로 확인
...
- [x] K6 시나리오 — 300명 동시 진입 → 폴링 → 주문까지 **관통 여정 1본**으로 확장(`queue-order-flow.js`, 계획의 queue-enter.js를 대체 — DB 보호 검증엔 주문까지 필요). 유저·상품 시드는 `seed.sh`, 서버 지표(대기 인원·Hikari·CPU)는 `collect-metrics.sh`
- [x] 볼 것 — 발급 속도(드레인 기울기로 배치 속도 확인), 대기 인원 변화, 주문 API 에러율·DB 커넥션 사용률·**CPU 사용률** → 실측 결과 병목은 커넥션이 아니라 **요청당 BCrypt 인증 CPU**(포화 시 점유시간이 0.2s 가정 → 수 초로 부풂)
- [x] **계산과 실제가 다르면 배치 크기 재조정** — **14(풀 붕괴) → 7(점유시간 부풂으로 오히려 악화) → 3(분산·핫로우 모두 실패 0) 확정**, 이유와 함께 `reports/batch-sizing.md` 8·9절에 기록. `application.yml` 반영
- [x] **이론 → 실측 → 재조정 루프를 문서화** — Stage 2 초기값과 실측치를 나란히 두고 최종 배치(3)·TTL(300초 유지) 확정 근거를 `reports/batch-sizing.md`에 남김. 부하 중 **진입-발급 레이스**(순번 응답 전에 스케줄러가 발급 → 엉뚱한 404)도 발견해 수정 — 실측이 아니면 못 잡았을 버그
- [x] 대기열의 평탄화 효과 정리 — 300명 폭주가 대기열에 쌓였다가(피크 230명) 배치 속도로 일정하게 빠져나가는 드레인 곡선 확인(`reports/data/run*-metrics.csv`)

**이 단계 완료 기준:** 많이 몰려도 DB가 배치 속도로 보호되고, 배치 크기·만료 시간의 최종값이 **이론이 아니라 실제 측정으로** 논리적으로 확정된다. → 배치 3에서 300명 전원 주문 성공·HTTP 실패 0·pending ≤ 14로 충족

---

## Stage 8 — 병목 판별 실험: 정말 인증(BCrypt) 때문인가 (추가 실험)

> 쉽게 말하면: Stage 7의 결론("병목은 커넥션이 아니라 요청당 BCrypt 인증 CPU")은 정황 증거(CPU 100% 포화 + 점유시간 부풂)로 세운 **가설**이다. 인증 비용만 통제 변수로 바꿔 같은 부하를 다시 걸어, 이 가설을 직접 검증하는 단계.

- [x] **실험 설계** — 인증 결과 캐시를 통제 변수로. 나머지(시나리오·300 VU·상품 분산·폴링 주기)는 Stage 7과 동일 고정. 캐시는 기본 off 플래그로 토글해 같은 jar로 A/B
- [x] **대조 측정** — 배치 14 고정, 캐시 off(E1)/on(E2). 결과: Hikari pending 피크 **41 → 0**, 주문 지연 **3.25s → 264ms**, 처리량 **58 → 167/s**. 인증 CPU만 걷어내니 커넥션 굶주림 소멸
- [x] **판별 기준** — E1 vs E2 대조로 **가설 채택**: 배치 3이 필요했던 범인은 요청당 BCrypt 인증 CPU. E3(배치 30)은 처리량 이득 0·active 39로 천장 근접 → 인증 제거 후 병목은 진입 스톰+주문 비용으로 이동, sweet spot은 14 부근
- [x] **배치 최종값 재확정** — 인증 구조를 안 바꾸는 한 **3 유지**. 판별·근거·데이터(runE1~E3)는 `reports/batch-sizing.md` 10절에 기록

**이 단계 완료 기준(달성):** "배치 3이 필요했던 이유가 인증 CPU였다"가 대조 측정으로 판별됐고, 배치 확정값의 전제(9절 전제 1)가 실험으로 뒷받침됨.

> **처방은 보류(범위 밖).** 정석 해법은 비밀번호 캐싱이 아니라 **첫 인증 후 서명된 신원 토큰 발급 → 이후 요청은 서명만 검증**(폴링 스톰 제거, 비밀번호 파생물 미저장). 실험 캐시는 보안 후퇴(솔트 없는 빠른 해시 상주)라 제품 코드로 채택하지 않고 되돌림. 토큰 도입은 별도 기능 단위로 남긴다.
...
docs/volume-8/
  ...
  measurement/k6/
    seed.sh               ← 측정용 기본 데이터 (유저 300 + 상품 10)
    queue-order-flow.js   ← 진입→폴링→주문 관통 시나리오 (계획의 queue-enter.js를 확장 대체)
    collect-metrics.sh    ← 대기 인원·Hikari·CPU 0.5초 간격 수집기
  reports/
    batch-sizing.md       ← 배치 크기 근거 + 실측 조정 기록 (8·9절 = Stage 7 실측)
    data/                 ← 회차별 K6 요약·서버 지표 CSV 원자료
```
