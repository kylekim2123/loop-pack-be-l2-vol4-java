...
- [ ] **실험 설계** — 인증 비용만 낮춘 조건(예: 인증 결과 캐시, BCrypt 강도 하향 등 중 택1)을 만들고, 나머지(시나리오·300 VU·상품 분산·폴링 주기)는 Stage 7과 동일하게 고정
- [ ] **대조 측정** — 인증 비용을 낮춘 상태에서 **배치 14(이론값)로 재측정**. 인증이 진범이면 CPU 포화가 사라지고 점유시간이 0.2초 가정으로 돌아와 pending·실패 없이 버텨야 한다
- [ ] **판별 기준** — Stage 7 회차와 나란히 비교: CPU 사용률 곡선, Hikari pending, 주문 지연(점유시간 대리 지표), HTTP 실패율. 가설 채택/기각을 명시적으로 기록
- [ ] **배치 최종값 재확정** — 인증 병목이 입증되고 제거 가능하다면 배치 상향 여지를, 아니라면 3 유지 근거를 `reports/batch-sizing.md`에 이어 기록

**이 단계 완료 기준:** "배치 3이 필요했던 이유가 인증 CPU였다/아니었다"가 추측이 아니라 대조 측정으로 판별되고, 배치 확정값의 전제가 실험으로 뒷받침된다.
