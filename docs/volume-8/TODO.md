...
- [ ] **`POST /queue/enter`** — 대기열 명단에 추가(`ZADD`)하고 현재 순번을 응답. 순서 기준 숫자는 "지금 시각(밀리초)"
- [ ] **중복 진입 막기** — 명단엔 이름이 하나만 들어가므로 두 번 안 세워짐. 새로고침하면 순번만 갱신(뒤로 밀림)
- [ ] **`GET /queue/position`** — 내 순번 조회(`ZRANK`). (Stage 5에서 예상 대기시간·입장권 포함으로 확장)
- [ ] **전체 대기 인원** — 명단 전체 인원 수 조회(`ZCARD`)
- [ ] 코드 배치 — `domain.queue`에 대기열 개념, `infrastructure`에 Redis 접근 코드
- [ ] 대기열에 없는 유저가 순번 조회하면 `QUEUE_NOT_FOUND` 에러
