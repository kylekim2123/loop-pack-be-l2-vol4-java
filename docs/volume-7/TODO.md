...
- [x] k6 시나리오 — **plan 수정(규모 조정):** 1만 → 수량 100장에 **300명 동시 요청**(과제 기준 100명을 넘는 최소 규모, 로컬 부하 고려) → `measurement/k6/coupon-issue.js` + `seed.sh`
- [x] 관찰 — 접수 300건 전원 202 즉시 응답(4.9초 창), Consumer 42초 순차 소진, 최종 발급 **정확히 100 / 초과 0 / 중복 0**, FAILED 200(전부 수량 소진 사유), Outbox 미발행 잔여 0 → `reports/coupon-concurrency.md`
- [x] Kafka 버퍼 효과 서술 — 접수 창(4.9초)과 소진 창(42초)의 시간 분리로 관찰. 접수 지연은 큐잉이 아닌 요청별 BCrypt 인증이 지배(리포트에 한계 명시)

**검증:** 300 동시 요청에도 발급은 정확히 100장, 초과·중복 0건. API는 접수만 하고 즉시 응답. ✅
