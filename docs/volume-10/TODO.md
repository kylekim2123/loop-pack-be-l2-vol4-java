...
- [x] **체크리스트 매핑 확인** — 4개 항목 각각을 어떤 코드·테스트가 덮는지 `검증-리포트.md §2`에 1:1 매핑. 신뢰 검증 4종(기간격리·멱등·동점·TOP100)은 §3에 무엇을·어떻게·결과로 정리
- [x] **E2E 관통 (수동)** — `http/commerce-api/ranking-v1.http`에 **[Vol10] 주간·월간 배치 관통** 구간 추가(일간 시나리오에 이어붙임): 원장 심기 → `weeklyRankingJob`/`monthlyRankingJob` 실행 명령 → period별 조회(주간·월간·같은 주 다른 날짜·미발행 빈 목록·일간 회귀·잘못된 period 400). 절차 전문은 `검증-리포트.md §5`
- [x] **운영 cron 설계 문서화** — `검증-리포트.md §7`에 권장 스케줄(주간 월 01:00 / 월간 1일 02:00 KST)·실행 명령·"왜 앱 내 @Scheduled가 아닌가"(책임 분리 + JobInstance 중복 방지, 결정 ⑥)
- [x] **회귀** — `./gradlew test` **BUILD SUCCESSFUL** (전 모듈). Volume 10 랭킹 자동 테스트 46개 전부 통과(기간계산 14·점수 3·스키마 7·주간Job 5·월간Job 3·API E2E 14), 일간 랭킹·기존 배치·streamer 회귀 없음. 결과 요약은 `검증-리포트.md §4·§6`

**이 단계 완료 기준:** 과제 체크리스트 4개가 전부 채워지고, 이벤트 발생부터 월간 랭킹 조회까지 전 구간을 명령 순서대로 재현할 수 있다. ✅ (검증-리포트.md로 남김)
