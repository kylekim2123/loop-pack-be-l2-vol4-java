...
- **cleanup(Tasklet)**: 스테이징에서 이번 기간 행 DELETE — 지난 실패 실행의 잔재가 섞이지 않게.
- **aggregate(Chunk)**: Reader가 `metric_date BETWEEN + GROUP BY product_id`로 **상품당 1행(기간 합계)** 씩 읽고(JdbcCursorItemReader — GROUP BY로 접힌 결과를 순차 스트리밍하는 데 커서가 적합), Processor가 **Java로 점수 계산**(결정 ②의 계산기), Writer가 **스테이징 테이블에 적재**(아직 순위 없음). 전 상품을 메모리에 들지 않고 DB에 내려놓는 이유 — 순위를 위해 인메모리로 전량 수집하면 상품 수가 커질 때 OOM으로 가는 길이라서요.
...
- [ ] **aggregate Step (Chunk)** — Reader: `JdbcCursorItemReader`, `SELECT product_id, SUM(...) FROM product_metrics_daily WHERE metric_date BETWEEN ? AND ? GROUP BY product_id` (점수 계산 없음 — SQL엔 합산만) / Processor: 기간 점수 계산기 호출 / Writer: 스테이징 INSERT. chunk 크기 500(전송·커밋 균형, 근거와 함께 상수화)
